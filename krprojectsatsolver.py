...
def read_dimac(filename):
    ...
    for line in open(filename):
        if line.startswith('c'): # ignore comment lines starting with 'c'
            continue
        if line.startswith('p'): # gathering number of variables and number of clauses from line starting with 'p'
            num_vars = line.split()[2] 
            num_clauses = line.split()[3]
            num_vars, num_clauses = int(num_vars), int(num_clauses) # transforming into integers
            continue
        c = [int(x) for x in line[:-2].split()] # gathering clauses as integers for every line in the document, also removes '0's 
        clauses.append(c)
    return clauses, num_vars, num_clauses

test_read = read_dimac(sudoku_example_path) # reads the example sudoku as integers in lists.
...
cnf_formula = dict(zip(range(1, num_clauses + 1), clauses)) # making a dictionary with the numbers of clauses 
...
def get_clause_dictionary(cnf_formula):
  ...
  for clause in cnf_formula:
    ...
    for variable in clause:
      ...
      if variable_name in dictionary: dictionary[variable_name].append(clause_index)
      #Else add it to the dict referring to this clause
      else: dictionary[variable_name] = [clause_index]
  return dictionary
...
def get_rand_var(cnf_formula):
  ...
  clause  = []
  while len(clause) == 0:
    warnings.warn("Empty clause when retrieving random varaible. Will be found in the next consistency check.")
...
def get_variable_by_heuristic(cnf_formula, heuristic, current_variable_assignment):
  ...
  warnings.warn("{0} was not implemented yet or an invalid heuristic. Falling back to random varaible selecion".format(heuristic))
  while true:
      variable = get_rand_var(cnf_formula)
      if not variable in current_variable_assignment:
        return variable
...
def SAT_simplify(cnf_formula, current_variable_assignment, var_assignment_history, log_level):
  ...
  if log_level > 1:
    print("Removed {0} tautologies from the CNF. CNF length reduced number of clauses to {1} clauses".format(len(tautologies), len(cnf_formula)))
  ...
  if log_level > 1:
    print("Removed {0} unit clauses from the CNF. Removed {1} clauses that became true by setting the unitclause to true. Changed {2} clauses according to the unit clause varaible. CNF length reduced number of clauses to {3} clauses".format(len(unit_clauses), removed_clauses, changed_clauses, len(cnf_formula)))
  ...
  if log_level > 1:
    print("Removed {0} pure literal clauses from the CNF. CNF length reduced number of clauses to {1} clauses".format(len(pure_literal_clauses), len(cnf_formula)))

  nothing_changed = removed_clauses is 0 and changed_clauses is 0 and len(unit_clauses) is 0 and len(tautologies) is 0
...
def full_SAT_step(cnf_formula,history, log_level, current_variable_assignment,var_assignment_history):
  ...
  while not simplification_exhausted:
    cnf_formula, simplification_exhausted= SAT_simplify(cnf_formula, current_variable_assignment,var_ass_history, log_level)
    if log_level>1 and not simplification_exhausted:
      print("Succesfully simplified formula. Continuing to simplify.")
    if log_level>0 and simplification_exhausted:
  ...
  current_variable_assignment[abs(random_variable)] = bool(random_variable/abs(random_variable)+1)
...
def SAT_check_and_backtrack(cnf_formula, history, current_variable_assignment, var_assignment_history, log_level):
  ...
  if log_level > 1:
    print("\n Formula is consistent: {0}, has empty clauses: {1} backtracking: {2}".format(consistent, has_empty_clauses, not consistent or has_empty_clauses))
